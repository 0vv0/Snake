...
public class Board extends AbstractBoard<Elements> {
    ...
    public List<Elements> getNear(int x, int y, int radius) {
        ...
        for (int layer = 0; layer < this.countLayers(); ++layer) {
            result.addAll(this.getNear(layer, x, y, radius));
        }
    ...
    protected List<Elements> getNear(int numLayer, int x, int y, int radius) {
        ...
        for (int dx = -radius; dx <= radius; ++dx) {
            for (int dy = -radius; dy <= radius; ++dy) {
                if (!PointImpl.pt(x + dx, y + dy).isOutOf(this.size) && (dx != 0 || dy != 0) && (!this.withoutCorners() || dx == 0 || dy == 0)) {
                    result.add(this.getAt(numLayer, x + dx, y + dy));
                }
            }
        }
